...
import { FindOptionsWhere, In, LessThanOrEqual, MoreThanOrEqual, Raw } from 'typeorm';
...
export const getPromoByMicrositeId = async (req: Request, res: Response, next: NextFunction) => {
  try {
    const promoMicrositeId = Number(req.params.promomicrositeId) as number;
    ...
    const promoids = JSON.parse(promoMicrosite.promo_id);
    const [promo, count] = await promoRepo.findAndCount({
      where: {
        id: In(promoids),
      },
